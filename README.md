...
| 转移 | 导出 `.note` / PDF（带页眉页码）/ 资料库 `.notebook` / 笔记文字 `.txt`  / 富文本 `.rtf` / Markdown `.md` / 当前页图片 / 当前页 PDF / 整篇长图 / 资料库合并 PDF，导入笔记文件（导入前自动备份）；支持 ESC 关闭弹窗；支持 iPadOS「分享」发送到「文件」/隔空投送；快捷键 Ctrl+S 保存 / Ctrl+N 新建 / Ctrl+D 复制页 / Ctrl+F 查找 / Ctrl+G 大纲 |
