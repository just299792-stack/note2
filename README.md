...
| 纸张 | 纸张颜色可自定义（任意背景色，自动推导默色与深浅适配），纸张大小（标准/宽版，切换时自动缩放过程内容），空白 / 横线 / 方格 / 点阵 / 康奈尔 / 清单 / 计划表 / 故事板 / 乐谱 / 法律纸 × 6 色（细腻纸纹），10 种纸张全部可选，行距可调；模板中心同时展示内置模板与自定义模板，内置 7 个常用模板 + 自定义模板，新建可选模板、可插入/应用、一键存为模板；PDF/图片背景透明度可调 |
