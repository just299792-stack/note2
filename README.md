...
| 组织 | 新建菜单内置模板（课程笔记/会议纪要/读书笔记/周计划/待办清单，含标题文字），科目 → 笔记本 → 笔记（笔记本可自定义封面颜色/符号，笔记可加颜色标签并筛选），「最近」快捷视图汇总跨笔记本最近编辑；笔记列表带首页缩略图；删除的笔记进入回收站可恢复，多选可批量移动/置顶/标签/删除；全局搜索（ESC 快速清除）+ 笔记内查找+替换 + 大纲（分级）+ 统计（含录音/附件/平均字数） + 页面拖拽排序 |
