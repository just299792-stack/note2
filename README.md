...
| 录音 | 每篇笔记录音/回放/删除（需 https 环境），录音按钮脉冲提示，面板显示录音数与总时长，波形预览、点击波形定位，回放调速 0.5x–2x、跨页自动翻页跟随，回放时笔迹随时间重画，录音时自动记录翻页事件（回放时按时间线自动切页），每段录音可单独导出为音频文件，可一键清空全部录音；支持朗读当前页文字、朗读整篇笔记（跨页自动翻页）（中文 TTS，逐句朗读并高亮当前句的文字框，速度可调、可暂停/继续） |
