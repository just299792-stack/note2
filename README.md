...
| 工具 | 文字输入（样式条：字号/加粗/斜体/下划线/对齐/颜色/预设）、形状、12+ 颜色、粗细可调、插入图片/拍照、PDF 导入标注 |
| 手势 | 双指缩放平移、双指轻点撤销、写至页尾自动翻页 |
| 界面 | 工具条三布局（顶部/左侧/底部）自由切换，设置中心分栏分类，6 套主题强调色（蓝/紫/粉/绿/橙/石墨），渐变玻璃质感 |
| 录音 | 每篇笔记录音/回放/删除（需 https 环境），波形预览、点击波形定位，回放时笔迹与音频同步重现、跨页自动翻页跟随；支持朗读当前页文字（中文 TTS） |
| 纸张 | 空白 / 横线 / 方格 / 点阵 / 康奈尔 / 清单 / 计划表 / 故事板 / 乐谱 / 法律纸 × 6 色；模板中心内置听课/会议/待办/日程模板，也可把任意页存为自定义模板 |
| 组织 | 科目 → 笔记本 → 笔记（笔记本可自定义封面颜色），页面缩略图侧栏，多页管理；跨笔记本全局搜索 |
