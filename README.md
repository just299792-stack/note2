...
| 录音 | 每篇笔记录音/回放/删除（需 https 环境），波形预览、点击波形定位，回放调速 0.5x–2x、跨页自动翻页跟随；支持朗读当前页文字（中文 TTS，速度可调） |
| 纸张 | 空白 / 横线 / 方格 / 点阵 / 康奈尔 / 清单 / 计划表 / 故事板 / 乐谱 / 法律纸 × 6 色（细腻纸纹），行距可调；模板中心内置常用模板 + 自定义模板 |
| 组织 | 科目 → 笔记本 → 笔记（笔记本可自定义封面颜色/符号），笔记列表带首页缩略图，页面缩略图侧栏；跨笔记本全局搜索（命中高亮）+ 笔记内查找（Ctrl+F）+ 自动大纲 |
